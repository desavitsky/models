...
object Distances {
  ...
  def get[T](
      ...
      coordinateValues: T,
      distanceType: String
  ): RDD[Double] = (points, coordinateValues, distanceType) match {

    case (
          points: RDD[Array[Double] @unchecked],
          coordinateValues: Array[Double],
          "EuclideanDistance"
        ) =>
      getEuclideanDistance(points, coordinateValues)
    ...
    case (
          points: RDD[String @unchecked],
          coordinateValues: String,
  ...
  object LenvenshteinDistance {
    ...
    private def lenvenshteinDistance(
        array1: Array[String],
        array2: Array[String]
    ): (Double, Array[Int]) = {
    ...
    def dis(
      ...
      (
        (dis._1 - dis._2.filter(_ != 0).length * 0.5) / dis._2.length.toDouble,
        dis._2
      )
    }
  }

  private def getURL_LenvenshteinDistance(
      points: RDD[String],
      coordinateValues: String
  ): RDD[Double] = {
  ...
  private def getLenvenshteinDistance(
      points: RDD[String],
      coordinateValues: String
  ): RDD[Double] = {
    val distance = points.map { p =>
  ...
  private def getEuclideanDistance(
      points: RDD[Array[Double]],
      coordinateValues: Array[Double]
  ): RDD[Double] = {
    val distance = points.map { p =>
      Math.sqrt(
        p.zip(coordinateValues).map { x => Math.pow(x._1 - x._2, 2) }.sum
      )
    }
//    distance.checkpoint()
    distance
